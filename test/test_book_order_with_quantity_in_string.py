#Booking an order with quantity in string and verifying a popup message
...
def test_book_order_with_quantity_in_string():
    with sync_playwright() as p:
        ...
        print("Entering the quantity to string")
        
        page.get_by_role("textbox", name="Qty").click()
        page.get_by_role("textbox", name="Qty").fill("abc")
        ...
        heading = page.get_by_role("heading", name= "Invalid Quantity").inner_text()
        message = page.locator(".msgprint").inner_text()
        
        assert "Invalid Quantity" in heading

        assert f"Quantity for Item {product_code} cannot be zero" in message
        
        print("Quantity in string is verified successfully.")
