#Increasing the quantity and verifying the amount is equal to product of 
#one item price and increased quantity.
...
def test_book_order_increase_quantity():
    with sync_playwright() as p:
        ...
        float_one_item= float(one_item_amount.replace(",", ""))
        print(f"amount of one item in float {float_one_item}") 
        ...
        expected_amount = float_inc_qty * float_one_item
        print(f"expected amount {expected_amount}")
        
        print("Verifying price after increasing")

        assert round(float_inc_item_amount, 3) == round(expected_amount, 3), \
    f"Mismatch: Amount after inc items({round(inc_item_amount,3)}) != amount({round(expected_amount,3)})"
        print("Verifying price after increasing passed successfully.")
